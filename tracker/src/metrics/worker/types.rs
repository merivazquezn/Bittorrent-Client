...
impl MetricsWorker {
    ...
    fn get_lower_bound(record_vector: &Vec<(i32,DateTime<Local>)>, timeframe: TimeFrame) -> usize {
        let total_minutes =  match timeframe {
            TimeFrame::LastDays(n_days)=> {
                (n_days * 24 * 60) as usize
            }
            TimeFrame::LastHours(n_hours)=> {
                (n_hours * 60) as usize
            }
        };
    ...
    fn rounded_timestamp_string(timestamp: DateTime<Local>, round_to_duration: Duration) -> String {
        timestamp.duration_round(round_to_duration).unwrap().format("%Y-%m-%d %H:%M:%S").to_string()
    }

    fn group_slice(record_slice: &[(i32,DateTime<Local>)], groupby: GroupBy, metric_key: String) -> Vec<(i32,String)>{
        let group_minutes;
        let round_to;
        match groupby{
    ...
    fn send_metric(
        ...
        if !self.record.contains_key(&metric_key){
            let _ = https_service.send_ok_response("".as_bytes().to_vec(), "application/json".to_string());
            return;
        }
        ...
        let _ = https_service.send_ok_response(json.as_bytes().to_vec(), "application/json".to_string());
    }
