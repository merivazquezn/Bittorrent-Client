...
impl MetricsController {
    pub fn handler_metrics(
        ...
        let json: String =
            metrics.get_metrics_response(metric_key.to_owned(), time_frame, groupby)?;
    ...
    pub fn get_torrents(
        ...
        let json: String = metrics.get_torrents()?;
        http_service.send_ok_response(json.as_bytes().to_vec(), "application/json".to_string())?;
        Ok(())
    }

    fn get_requested_time_frame(
        time_frame_count_key: &str,
        time_frame_key: &str,
    ) -> Result<TimeFrame, MetricsError> {
        ...
        match time_frame_key {
            ...
            &_ => Err(MetricsError::InvalidTimeFrameReceived(
                time_frame_key.to_string(),
            )),
        }
    }

    fn get_requested_groupby(
        groupby_count_key: &str,
        groupby_key: &str,
    ) -> Result<GroupBy, MetricsError> {
        ...
        match groupby_key {
            ...
            &_ => Err(MetricsError::InvalidGroupByReceived(
                groupby_key.to_string(),
            )),
        }
    }
}
